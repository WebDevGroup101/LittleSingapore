...
function draw()
{
    ...
    // spaceship_b = {x: mouseX, y: mouseY, radius: 50};
    
    var collide = checkCollision(spaceship_a, spaceship_b, spaceship_a.radius + spaceship_b.radius);
...
function getSpeed(spaceship)
{
    ...
    if (spaceship.x < 0 || spaceship.x > width) {
        speed.x *= -1;
    } else if  (spaceship.y < 0 || spaceship.y > height) {
        speed.y *= -1;
    }
